...
class SignModel(nn.Module):
    ...
    def run_batch(
        ...
        if self.do_recognition:
            ...
            # N x T x C
            gloss_scores = self.gloss_output_layer(encoder_output)
            # T x N x C, as CTC decoding expects time first
            gloss_probabilities = gloss_scores.log_softmax(2).permute(1, 0, 2)
            decoded_gloss_sequences = ctc_decode(
                log_probs=gloss_probabilities.detach(),
                lengths=batch.sgn_lengths,
                beam_size=recognition_beam_size,
                blank=self.gls_vocab.stoi[SIL_TOKEN],
            )
        else:
...
def build_model(
    ...
    if encoder_type != "transformer":
        raise ValueError(
            f"Unsupported encoder type {encoder_type!r}: this release is transformer only."
        )
    assert (
    ...
    if do_translation:
        ...
        decoder_type = cfg["decoder"].get("type", "transformer")
        if decoder_type != "transformer":
            raise ValueError(
                f"Unsupported decoder type {decoder_type!r}: this release is transformer only."
            )
