...
class Vocabulary:
    ...
    def __init__(self):
        # stoi grows with unknown tokens; don't use it for saving or size
        self.specials = []
        self.itos = []
        self.stoi = None
...
def build_vocab(
    field: str, max_size: int, min_freq: int, dataset: Iterable, vocab_file: str = None
) -> Vocabulary:
    """
    Builds vocabulary for `field` from the given `dataset` or `vocab_file`.

    :param field: either "gls" or "txt"
    :param max_size: maximum size of vocabulary
    :param min_freq: minimum frequency for an item to be included
    :param dataset: dataset exposing `.examples` with `.gls` / `.txt` token lists
