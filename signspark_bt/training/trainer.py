...
from ..data import SignBackTranslationDataset, load_data, make_data_iter
from ..data.batch import Batch
from ..data.vocabulary import SIL_TOKEN
from ..helpers import (
    log_data_info,
    load_config,
    log_cfg,
...
from .prediction import test, validate_on_data
...
class TrainManager:
    ...
    def __init__(self, model: SignModel, config: dict) -> None:
        ...
        self.logger.info("Writing checkpoints and logs to %s", self.model_dir)
        # initialize wandb
        self.wb_writer, _ = init_wandb(config, self.logger)
    ...
    def _save_checkpoint(self, is_best: bool = False) -> None:
        ...
        :param is_best: also copy this checkpoint to best.ckpt, which is what
            the inference API loads
        """
        model_path = f'{self.model_dir}/{self.steps}.ckpt'
        state = {
            "steps": self.steps,
        ...
        if is_best:
            # A copy rather than a symlink: the checkpoint queue may later
            # delete the numbered file this was promoted from.
            best_path = os.path.join(self.model_dir, "best.ckpt")
            shutil.copyfile(model_path, best_path)
            self.logger.info("Saved new best checkpoint to %s", best_path)
    ...
    def train_and_validate(
        self,
        train_data: SignBackTranslationDataset,
        valid_data: SignBackTranslationDataset,
    ) -> None:
        """
        ...
        for epoch_no in range(self.epochs):
            ...
            for batch in iter(train_iter):
                ...
                if self.steps % self.logging_freq == 0 and update:
                    ...
                    if self.wb_writer is not None:
                        self.wb_writer.log({"Steps": self.steps,
                                            "Epoch": epoch_no + 1,
                                            "Batch Recognition Loss": recognition_loss,
                                            "Batch Translation Loss": translation_loss,
                ...
                if self.steps % self.validation_freq == 0 and update:
                    ...
                    if self.is_best(ckpt_score):
                        ...
                        self.logger.info(
                            "New best validation result [%s]: %.4f",
                            self.early_stopping_metric,
                            ckpt_score,
                        )
                    ...
                    if self.wb_writer is not None:
                        self.wb_writer.log({"Epoch": epoch_no + 1,
                                            "Step": self.steps,
                                            "Valid Recognition Loss": val_res["valid_recognition_loss"] if self.do_recognition else -1,
                                            "Valid Translation Loss": val_res["valid_translation_loss"],
                                            "Valid PPL": val_res["valid_ppl"],
                                            "Valid WER": val_res["valid_scores"]["wer"] if self.do_recognition else -1,
                                            "Valid WER Del": val_res["valid_scores"]["wer_scores"]["del_rate"] if self.do_recognition else -1,
                                            "Valid WER Ins": val_res["valid_scores"]["wer_scores"]["ins_rate"] if self.do_recognition else -1,
                                            "Valid WER Sub": val_res["valid_scores"]["wer_scores"]["sub_rate"] if self.do_recognition else -1,
                                            "Valid BLEU-1": val_res["valid_scores"]["bleu_scores"]["bleu1"],
                                            "Valid BLEU-2": val_res["valid_scores"]["bleu_scores"]["bleu2"],
                                            "Valid BLEU-3": val_res["valid_scores"]["bleu_scores"]["bleu3"],
                                            "Valid BLEU-4": val_res["valid_scores"]["bleu_scores"]["bleu4"],
                                            "Valid CHRF": val_res["valid_scores"]["chrf"],
                                            "Valid ROUGE": val_res["valid_scores"]["rouge"]})

                    self._log_examples(
                        sequences=[e.sequence for e in valid_data.examples],
                    ...
                    valid_seq = [e.sequence for e in valid_data.examples]
    ...
    def _log_parameters_list(self) -> None:
        ...
        self.logger.info("Trainable parameter tensors: %d", len(trainable_params))
        self.logger.debug("Trainable parameters: %s", sorted(trainable_params))
...
def train(cfg_file: str, change_cfg: dict) -> None:
    """
    ...
    cfg = change_config(cfg, change_cfg)
    
    # make config name the folder to save checkpoints etc.
    name = cfg.get('name', '')
    save_dir = Path(cfg['training'].get('model_dir', './'))
    if name != save_dir.name:
        save_dir = save_dir / name
    cfg['training']['model_dir'] = str(save_dir)
    ...
    shutil.copy2(cfg_file, trainer.model_dir + "/init_config.yaml")
    cfg_file = save_config(cfg, trainer.model_dir)
    ...
    wb_writer = trainer.wb_writer
    del trainer
    test(cfg_file, ckpt=ckpt, output_path=output_path, logger=logger, wb_logger=wb_writer)
