...
from ..data.vocabulary import PAD_TOKEN, SIL_TOKEN
from ..helpers import (
    bpe_postprocess,
    load_config,
...
from ..metrics import bleu, chrf, rouge, score_translations, wer_list
...
def validate_on_data(
    model: SignModel,
    data: SignBackTranslationDataset,
    batch_size: int,
    use_cuda: bool,
    sgn_dim: int,
    do_recognition: bool,
    ...
    with torch.no_grad():
        ...
        # for valid_batch in tqdm(iter(valid_iter), total=len(valid_iter)):
            batch = Batch(
                is_train=False,
        ...
        if do_recognition:
            ...
            # PHOENIX glosses carry annotation markers that are stripped
            # before scoring; other datasets are compared as written.
            if dataset_version == "phoenix_2014_trans":
                ...
                gls_cln_fn = lambda gloss: gloss

            gls_ref = [gls_cln_fn(" ".join(e.gls)) for e in data.examples]
            ...
            txt_bleu = bleu(references=gls_ref, hypotheses=gls_hyp)
        ...
        if do_translation:
            ...
            txt_ref = [join_char.join(e.txt) for e in data.examples]
            ...
            # TXT Metrics
            txt_scores = score_translations(
                references=txt_ref, hypotheses=txt_hyp, dataset_version=dataset_version
            )
            txt_bleu = txt_scores["bleu"]
            txt_chrf = txt_scores["chrf"]
            txt_rouge = txt_scores["rouge"]
...
def test(
    cfg_file, ckpt: str, output_path: str = None, logger: logging.Logger = None, wb_logger=None
) -> None:
    """
    ...
    if wb_logger is None:
        wb_logger, _ = init_wandb(cfg, logger)
    ...
    if wb_logger is not None:
        wb_logger.log({"DEV BLEU-1": dev_best_translation_result["valid_scores"]["bleu_scores"]["bleu1"],
                       "DEV BLEU-2": dev_best_translation_result["valid_scores"]["bleu_scores"]["bleu2"],
                       "DEV BLEU-3": dev_best_translation_result["valid_scores"]["bleu_scores"]["bleu3"],
                       "DEV BLEU-4": dev_best_translation_result["valid_scores"]["bleu_scores"]["bleu4"],
                       "DEV Rouge": dev_best_translation_result["valid_scores"]["rouge"],
                       })
    ...
    if wb_logger is not None:
        wb_logger.log({"TEST BLEU-1": test_best_result["valid_scores"]["bleu_scores"]["bleu1"],
                       "TEST BLEU-2": test_best_result["valid_scores"]["bleu_scores"]["bleu2"],
                       "TEST BLEU-3": test_best_result["valid_scores"]["bleu_scores"]["bleu3"],
                       "TEST BLEU-4": test_best_result["valid_scores"]["bleu_scores"]["bleu4"],
                       "TEST Rouge": test_best_result["valid_scores"]["rouge"],
                       })
    ...
    if output_path is not None:
        if do_recognition:
            ...
            _write_to_file(
                ...
                [e.sequence for e in dev_data.examples],
            ...
            _write_to_file(
                ...
                [e.sequence for e in test_data.examples],
