...
def score_translations(
    references: list, hypotheses: list, dataset_version: str = "", level: str = "word"
) -> dict:
    """BLEU, chrF and ROUGE over detokenized sentences.

    Applies the dataset's scoring convention: Chinese is scored per character,
    so references and hypotheses are split into characters first.

    :param references: one reference sentence per sample
    :param hypotheses: one hypothesis sentence per sample
    :param dataset_version: e.g. "csldaily", selects the convention
    :param level: "word", "bpe" or "char"
    :return: {"bleu": {...}, "chrf": float, "rouge": float}
    """
    assert len(references) == len(hypotheses), (len(references), len(hypotheses))

    if "csldaily" in dataset_version.lower():
        references = [" ".join(list(t.replace(" ", ""))) for t in references]
        hypotheses = [" ".join(list(t.replace(" ", ""))) for t in hypotheses]

    return {
        "bleu": bleu(references=references, hypotheses=hypotheses),
        "chrf": chrf(references=references, hypotheses=hypotheses),
        "rouge": rouge(references=references, hypotheses=hypotheses),
    }
